...
ALTER TABLE `subscriptions` RENAME TO `__old_subscriptions`;--> statement-breakpoint
CREATE TABLE `subscriptions` (
	...
	CONSTRAINT "subscriptions_price_nonnegative" CHECK("price_minor" >= 0),
	CONSTRAINT "subscriptions_importance_range" CHECK("importance" BETWEEN 1 AND 5),
	CONSTRAINT "subscriptions_satisfaction_range" CHECK("satisfaction" IS NULL OR "satisfaction" BETWEEN 1 AND 5),
	CONSTRAINT "subscriptions_billing_cycle_values" CHECK("billing_cycle" IN ('weekly', 'monthly', 'yearly')),
	CONSTRAINT "subscriptions_usage_level_values" CHECK("usage_level" IN ('often', 'sometimes', 'rarely', 'unknown')),
	CONSTRAINT "subscriptions_source_values" CHECK("source" IN ('manual', 'csv', 'email', 'store')),
	CONSTRAINT "subscriptions_status_values" CHECK("status" IN ('active', 'paused', 'cancelled'))
...
CREATE TABLE `__new_usage_checkins` (
	...
	CONSTRAINT "usage_checkins_level_values" CHECK("level" IN ('often', 'sometimes', 'rarely'))
);--> statement-breakpoint
CREATE TABLE `__new_charges` (
	...
	`created_at` text NOT NULL,
	FOREIGN KEY (`subscription_id`) REFERENCES `subscriptions`(`id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "charges_amount_nonnegative" CHECK("amount_minor" >= 0),
	CONSTRAINT "charges_source_values" CHECK("source" IN ('manual', 'csv', 'email'))
);--> statement-breakpoint
INSERT INTO `subscriptions`("id", "user_id", "name", "plan", "price_minor", "currency", "billing_cycle", "start_date", "next_billing_date", "category", "importance", "satisfaction", "usage_level", "last_used_date", "source", "status", "notes", "created_at", "updated_at") SELECT "id", "user_id", "name", "plan", "price_minor", "currency", "billing_cycle", "start_date", "next_billing_date", "category", "importance", "satisfaction", "usage_level", "last_used_date", "source", "status", "notes", "created_at", "updated_at" FROM `__old_subscriptions`;--> statement-breakpoint
...
DROP TABLE `__old_subscriptions`;--> statement-breakpoint
...
CREATE INDEX `idx_charges_user_subscription_date` ON `charges` (`user_id`,`subscription_id`,`charged_on`);
